...
# error Architecture not supported
	...
	static inline uint128_t add128(uint128_t x, uint128_t y)
	{
		...
		uint128_t res;
		unsigned char c = 0;
		c = _addcarry_u64(0, x.lo, y.lo, &res.lo);
		c = _addcarry_u64(c, x.hi, y.hi, &res.hi);
		return res;
#else
		asm("add    %q2, %q0\n\t"
			"adc    %q3, %q1\n\t"
...
# error Architecture not supported
	...
	static inline uint128_t add128(uint128_t x, uint64_t y)
	{
		...
		uint128_t res;
		unsigned char c = 0;
		c = _addcarry_u64(0, x.lo, y, &res.lo);
		c = _addcarry_u64(c, x.hi, 0, &res.hi);
		return res;
#else
		asm("add    %q2, %q0\n\t"
			"adc    $0, %q1\n\t"
...
#pragma unroll
	...
	{
		uint128_t res = 0;
		for (std::string::const_iterator iter = s.begin(); iter != s.end() && (int)*iter >= 48; iter++) {
			res = mul128(res, 10);
